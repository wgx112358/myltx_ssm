...
class GenerationConfig:
    ...
    ssm_window_blocks: int = 0  # Delayed-compression window size for streaming SSM state updates
    disable_post_chunk_ssm_compression: bool = False  # Diagnostic switch to isolate post-chunk SSM writes
...
@dataclass
class ValidationStreamState:
    """Optional stream state container for validation generation."""

    ssm_state: "SSMState | None" = None
    pending_video_chunks: list[Tensor] = field(default_factory=list)
    pending_audio_chunks: list[Tensor | None] = field(default_factory=list)


@dataclass
class _ChunkGenerationResult:
    video_output: Tensor
    audio_output: Tensor | None
    next_ssm_state: "SSMState | None"
    stream_video_tokens: Tensor | None
    stream_audio_tokens: Tensor | None


def _snapshot_stream_chunk(chunk: Tensor | None) -> Tensor | None:
    if chunk is None:
        return None
    return chunk.detach().clone()


def _update_delayed_stream_state(
    *,
    stream_state: ValidationStreamState,
    next_ssm_state: "SSMState | None",
    chunk_video_tokens: Tensor | None,
    chunk_audio_tokens: Tensor | None,
    window_blocks: int,
    disable_compression: bool,
    compress_evicted_tokens: Callable[..., object] | None,
) -> ValidationStreamState:
    """Match training-time delayed compression for streaming validation."""
    if window_blocks < 0:
        raise ValueError(f"ssm_window_blocks must be non-negative, got {window_blocks}")
    if len(stream_state.pending_video_chunks) != len(stream_state.pending_audio_chunks):
        raise ValueError("pending_video_chunks and pending_audio_chunks must stay aligned")

    stream_state.ssm_state = next_ssm_state

    if disable_compression:
        stream_state.pending_video_chunks.clear()
        stream_state.pending_audio_chunks.clear()
        return stream_state

    if chunk_video_tokens is not None:
        stream_state.pending_video_chunks.append(_snapshot_stream_chunk(chunk_video_tokens))
        stream_state.pending_audio_chunks.append(_snapshot_stream_chunk(chunk_audio_tokens))

    if stream_state.ssm_state is None or compress_evicted_tokens is None:
        return stream_state

    while len(stream_state.pending_video_chunks) > window_blocks:
        evicted_video = stream_state.pending_video_chunks.pop(0)
        evicted_audio = stream_state.pending_audio_chunks.pop(0)
        stream_state.ssm_state = compress_evicted_tokens(stream_state.ssm_state, evicted_video, evicted_audio)
...
class ValidationSampler:
    ...
    def generate(
        ...
        stream_state: ValidationStreamState | None = None,
        ssm_state: "SSMState | None" = None,
        return_stream_state: bool = False,
        return_ssm_state: bool = False,
    ) -> (
        tuple[Tensor, Tensor | None]
        | tuple[Tensor, Tensor | None, ValidationStreamState]
        | tuple[Tensor, Tensor | None, "SSMState | None"]
    ):
        """Generate a video (and optionally audio) sample.
        Args:
            config: Generation configuration
            device: Device to run generation on
            stream_state: Optional stream state container for chunked generation
            ssm_state: Optional direct SSM state (legacy shortcut to stream_state.ssm_state)
            return_stream_state: If True, returns updated ValidationStreamState as a third item
            return_ssm_state: If True, returns updated SSM state as a third item
        Returns:
            Default: (video, audio)
            Optional: (video, audio, updated stream/ssm state)
        """
        if return_stream_state and return_ssm_state:
            raise ValueError("Only one of return_stream_state or return_ssm_state can be True")

        device = torch.device(device) if isinstance(device, str) else device
        self._validate_config(config)

        if stream_state is not None and ssm_state is not None and stream_state.ssm_state is not ssm_state:
            raise ValueError("stream_state.ssm_state and ssm_state must reference the same state when both provided")

        current_stream_state = stream_state
        if current_stream_state is None and return_stream_state:
            current_stream_state = ValidationStreamState(ssm_state=ssm_state)

        current_ssm_state = current_stream_state.ssm_state if current_stream_state is not None else ssm_state
        ...
        next_ssm_state = result.next_ssm_state
        if current_stream_state is not None:
            next_stream_state = _update_delayed_stream_state(
                stream_state=current_stream_state,
                next_ssm_state=next_ssm_state,
                chunk_video_tokens=result.stream_video_tokens,
                chunk_audio_tokens=result.stream_audio_tokens,
                window_blocks=config.ssm_window_blocks,
                disable_compression=config.disable_post_chunk_ssm_compression,
                compress_evicted_tokens=getattr(self._transformer, "compress_evicted_tokens", None),
            )
            next_ssm_state = next_stream_state.ssm_state
        else:
            next_stream_state = None
            if not config.disable_post_chunk_ssm_compression:
                next_ssm_state = self._compress_chunk_into_stream_state(
                    ssm_state=next_ssm_state,
                    evicted_video=result.stream_video_tokens,
                    evicted_audio=result.stream_audio_tokens,
                )

        if return_stream_state:
            if next_stream_state is None:
                next_stream_state = ValidationStreamState(ssm_state=next_ssm_state)
            return result.video_output, result.audio_output, next_stream_state
        if return_ssm_state:
            return result.video_output, result.audio_output, next_ssm_state
        return result.video_output, result.audio_output
    ...
    def _generate_standard(
        ...
        # Run denoising loop
        video_state, audio_state, next_ssm_state = self._run_denoising(
            config=config,
        ...
        stream_video_tokens = video_state.latent
        stream_audio_tokens = audio_state.latent if audio_state is not None else None
        ...
        return _ChunkGenerationResult(
            video_output=video_output,
            audio_output=audio_output,
            next_ssm_state=next_ssm_state,
            stream_video_tokens=stream_video_tokens,
            stream_audio_tokens=stream_audio_tokens,
        )
    ...
    def _generate_with_reference(
        ...
        # Run denoising loop
        combined_state, audio_state, next_ssm_state = self._run_denoising(
            config=config,
        ...
        return _ChunkGenerationResult(
            ...
            audio_output=audio_output,
            next_ssm_state=next_ssm_state,
            stream_video_tokens=target_latent,
            stream_audio_tokens=stream_audio_tokens,
        )

    def _compress_chunk_into_stream_state(
        self,
        ssm_state: "SSMState | None",
        evicted_video: Tensor | None,
        evicted_audio: Tensor | None,
    ) -> "SSMState | None":
        """Optionally compress current chunk tokens into stream state."""
        if ssm_state is None:
            return None

        compress_evicted_tokens = getattr(self._transformer, "compress_evicted_tokens", None)
        if compress_evicted_tokens is None:
            return ssm_state

        return compress_evicted_tokens(ssm_state, evicted_video, evicted_audio)
    ...
    def _run_denoising(
        ...
        device: torch.device,
        ssm_state: "SSMState | None" = None,
    ) -> tuple[LatentState, LatentState | None, "SSMState | None"]:
        ...
        self._transformer.to(device=device, dtype=torch.bfloat16)
        ...
        with torch.autocast(device_type=str(device).split(":")[0], dtype=torch.bfloat16):
            for step_idx, sigma in enumerate(sigmas[:-1]):
                ...
                # Run model (positive pass) - only this pass commits next-step SSM state
                step_input_ssm_state = ssm_state
                pos_video, pos_audio, next_step_ssm_state = x0_model(
                    video=video,
                    audio=audio,
                    perturbations=None,
                    ssm_state=step_input_ssm_state,
    ...
    def _validate_config(self, config: GenerationConfig) -> None:
        ...
        if config.ssm_window_blocks < 0:
            raise ValueError(f"ssm_window_blocks must be non-negative, got {config.ssm_window_blocks}")
