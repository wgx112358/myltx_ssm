...
class X0Model(torch.nn.Module):
    ...
    def forward(
        ...
        tuple[torch.Tensor | None, torch.Tensor | None]
        | tuple[torch.Tensor | None, torch.Tensor | None, "SSMState | None"]
    ):
        """
        Denoise the video and audio according to the sigma.
        Returns:
            Denoised video and audio, plus optional next SSM state.
        """
        if perturbations is None:
            source = video if video is not None else audio
            if source is None:
                raise ValueError("Either video or audio must be provided")
            perturbations = BatchedPerturbationConfig.empty(source.latent.shape[0])

        model_out: tuple[torch.Tensor | None, torch.Tensor | None] | tuple[torch.Tensor | None, torch.Tensor | None, "SSMState"]
        if return_ssm_state or ssm_state is not None:
            try:
                model_out = self.velocity_model(
                    video=video,
                    audio=audio,
                    perturbations=perturbations,
                    ssm_state=ssm_state,
                ...
                model_out = self.velocity_model(video, audio, perturbations)
        else:
            model_out = self.velocity_model(video, audio, perturbations)

        next_ssm_state = ssm_state
        if len(model_out) == 3:
            vx, ax, next_ssm_state = model_out
        elif len(model_out) == 2:
            vx, ax = model_out
        else:
            raise RuntimeError(f"Unexpected velocity_model output length: {len(model_out)}")

        denoised_video = to_denoised(video.latent, vx, video.timesteps) if vx is not None else None
        denoised_audio = to_denoised(audio.latent, ax, audio.timesteps) if ax is not None else None
        if return_ssm_state:
            return denoised_video, denoised_audio, next_ssm_state
        return denoised_video, denoised_audio
