#!/bin/bash
# 多 GPU 并行生成 ODE 数据（蒸馏模型版本）
# 支持两种模式：
# 1. 节点分片模式（推荐）
#    bash ode/launch_multi_gpu_distilled.sh <start_group> <node_gpu_count> <total_groups> [config_path]
#    - start_group: 当前节点从第几组开始处理，按 0 开始计数
#    - node_gpu_count: 当前节点可用 GPU 数量
#    - total_groups: 全局总分组数（通常等于总 GPU 数或你想切分的总 chunk 数）
#
#    示例：
#      bash ode/launch_multi_gpu_distilled.sh 2 6 100
#    含义：
#      - 整个 CSV 按 100 组切分
#      - 当前节点有 6 张 GPU
#      - 当前节点处理第 2~7 组
#      - 实际传给 Python 的 chunk_id 为 2~7，num_chunks 为 100
#
# 2. 兼容旧模式
#    bash ode/launch_multi_gpu_distilled.sh [gpu_count] [config_path]
#    - 等价于从第 0 组开始，总分组数 = 当前 GPU 数

set -euo pipefail

CONFIG_DEFAULT="ode/configs/gen_ode_data_distilled.yaml"
LOG_DIR="ode/logs_distilled"
...
usage() {
    echo "用法（节点分片模式）:"
    echo "  bash ode/launch_multi_gpu_distilled.sh <start_group> <node_gpu_count> <total_groups> [config_path]"
    echo ""
    echo "示例:"
    echo "  bash ode/launch_multi_gpu_distilled.sh 2 6 100"
    echo "  bash ode/launch_multi_gpu_distilled.sh 2 6 100 ode/configs/gen_ode_data_distilled.yaml"
    echo ""
    echo "兼容旧模式:"
    echo "  bash ode/launch_multi_gpu_distilled.sh [gpu_count] [config_path]"
}
...
for LOCAL_GPU_ID in $(seq 0 $((LAUNCH_COUNT - 1))); do
    ...
    CUDA_VISIBLE_DEVICES=${LOCAL_GPU_ID} python ode/gen_ode_data_distilled.py \
        --config "${CONFIG}" \
        --chunk_id "${CHUNK_ID}" \
